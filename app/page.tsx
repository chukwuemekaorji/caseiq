    ...
    <div className="min-h-screen bg-film text-ink">
      ...
      <header className="border-b border-ink/10 px-8 py-5 flex items-center gap-4">
        ...
        {caseData.caseDraft ? (
          <span className="font-mono text-xs text-graphite uppercase tracking-widest">
            {caseData.caseDraft.clientName}
            {caseData.caseDraft.matterNumber ? ` · ${caseData.caseDraft.matterNumber}` : ""}
          </span>
        ) : (
          <span className="font-mono text-xs text-graphite uppercase tracking-widest">
        ...
        {caseData.saveError && (
          <span className="font-mono text-xs uppercase tracking-widest text-amber" title="Working offline — changes aren't being saved right now, but nothing here is lost for this session.">
            Not saved · offline
          </span>
        )}
      ...
      <main className="mx-auto max-w-[1400px] px-8 py-12">
        {caseData.caseLoading ? (
          ...
          <CaseIntake onContinue={caseData.startCase} />
        ) : !caseData.parsed ? (
          ...
          <div className="space-y-10">
            ...
            {!selectedCluster && (
              <p className="text-center font-mono text-xs text-graphite">
                Select a point on the timeline to review that day&apos;s records.
              </p>
            )}
      ...
      {exhibit && (
        <ExhibitView
          ...
          incidentDate={caseData.incidentDate}
          caseName={caseData.caseDraft?.clientName ?? "Case"}
